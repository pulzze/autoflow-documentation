...
2. Then apply various actions to extract the value(s).
:::

## Transforming the data

1. First to split the log on a new line, use the action [string/split-newline](Documentation/actions-library/data/string/action-string-split-by-newline.mdx).
...
The objective is to extract the IP address and Port number. To do that, we can look for patterns of the **TCP connection** and **OUTSIDE:**. Then select the value that follows those patterns.

```
%ASA-6-302013: TCP connection 933 OUTSIDE:172.217.169.68
```

Similar to how the earlier [string/split-newline](Documentation/actions-library/data/string/action-string-split-by-newline.mdx) action was used to split the string on a new line;

An action to split the string by pattern [string/split-by](Documentation/actions-library/data/string/action-string-split-by.mdx) can be used to further isolate the value.  
