...
:::tip Required Concepts
Learn how to use [string/split-by-newline](../../../Documentation/actions-library/data/string/action-string-split-by-newline/) actions.
:::
...
- ITERATE: We will be iteration over the list of records that's saved in the scope `variable: out`.
- SCOPE: This is a special scope that existing in the iteration to record data as it loops through the array.
...
Similar to how the earlier [string/split-newline](Documentation/actions-library/data/string/action-string-split-by-newline.mdx) action split the string on a new line;

An action to split the string by pattern [string/split-by](Documentation/actions-library/data/string/action-string-split-by.mdx) will be used to further split the string to isolate the values.
